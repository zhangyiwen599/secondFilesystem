...
void FileSystem::LoadSuperBlock()
{
	...
	for (int i = 0; i < 2; i++)
	{
		int* p = (int *)&g_spb + i * 128;	//int为4字节，128*4刚好为521字节，为一个扇区大小

		//通过bufmanager读取磁盘的指定扇区
...
Inode* FileSystem::IAlloc()
{
	...
	if(sb->s_ninode <= 0)
	{
		...
		if(sb->s_ninode <= 0)
		{
			getLog().write(Log::ERROR,string("can't find free outer Inode"));
			return NULL;
		}
	}
...
Buf* FileSystem::Alloc()
{
	...
	{
		sb->s_nfree = 0;
		getLog().write(Log::WARNING,string("no free disk section"));
		return NULL;
	}
